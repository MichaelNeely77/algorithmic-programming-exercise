...
def merge(left, right):
    ...
    #Set current to the head fof the linked list
    current = merged.head
    ...
    #Iterate over left adnd right until we reach the tail node
    #of either
    while left_head or right_head:
    #if head_node of left is None, we're past the tail
    # Add the node from right to merged linked list
        if left_head is None:
            ...
            current.next_node = left_head.next_node
            #Call next on left to set loop condition to false
            left_head = left_head.next_node
        else:
        # Not at either tail node
            #Obtain node data to perfomr comparison operations
